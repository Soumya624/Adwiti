...
function add_csrftoken(data, tokenvalue) {
  if (data === undefined) data = {};
  if (tokenvalue === undefined) tokenvalue = window.csrftoken;
  if (tokenvalue === undefined) tokenvalue = $.cookie("csrftoken");
  if (!("csrftoken" in data)) data.csrftoken = tokenvalue;
  return data;
}
(function () {
  var orig_jsonp = $.jsonp;
  $.jsonp = function (params) {
    params.data = add_csrftoken(params.data);
    orig_jsonp(params);
  };
})();
...
(function ($) {
  ...
  var STR_ASYNC = "async",
    ...
    count = 0,
    lastValue,
    xOptionsDefaults = { callback: STR_JQUERY_JSONP, url: location.href },
    opera = win.opera,
    oldIE = !!$("<div>").html("\x3c!--[if IE]><i><![endif]--\x3e").find("i")
      .length;
...
jQuery.base64 = {
  ...
  encode: function (input) {
    ...
    while (i < input.length) {
      ...
      output =
        output +
        this._keys.charAt(enc1) +
        this._keys.charAt(enc2) +
        this._keys.charAt(enc3) +
        this._keys.charAt(enc4);
      chr1 = chr2 = chr3 = "";
      enc1 = enc2 = enc3 = enc4 = "";
    }
    return output;
...
window.isLocalOrPreProd =
  window.location.host.match(
    /^(localhost|prezi\.dev|.*preprod\.prezi\.com)$/
  ) !== null;
window.isProduction = !isLocalOrPreProd;
if (console === undefined) {
  var dummy = function (v) {};
  function _console() {
    return { log: dummy, info: dummy, warn: dummy, error: dummy };
  }
  var console = new _console();
}
window._LOGSERVICE_TOKEN = undefined;
(function ($) {
  $.extend({
    log: function (options) {
      if (!options) {
        isProduction || console.warn("no options given");
        return;
      }
      if (typeof options === "string") {
        ...
        if (!options.msg) {
          isProduction || console.warn("no log message given");
          return;
        }
      }
      if (isLocalOrPreProd) {
        if (typeof options.log_callback === "function") {
      ...
      var defaults = {
        ...
        levelname: "INFO",
        levelno: 20,
        created: parseInt(current_time / 1e3, 10),
        msecs: current_time % 1e3,
        relativeCreated: 0,
        msg: "",
        source: source,
        prezi_id: prezi_id,
      ...
      var _log_request = function () {
        $.jsonp({
          url: Site.LOG_SERVICE_ENDPOINT,
          cache: false,
          dataType: "jsonp",
          data: {
            format: "jsonp",
            data: data,
            prezi_id: settings.prezi_id,
            source: settings.source,
            authorization: window._LOGSERVICE_TOKEN,
            _method: "POST",
          ...
          error: function (xhr, textStatus) {
            isProduction ||
              console.warn(
                "backend problem encountered caused by: " + textStatus
              );
          },
        });
      };
      var _log = function () {
        if (window._LOGSERVICE_TOKEN === undefined) {
          var tokenUrl = Site.LOG_API_ENDPOINT + "/api/token/log/";
          $.getJSON(tokenUrl, function (data, status, xhr) {
            window._LOGSERVICE_TOKEN = data.header;
            _log_request();
          }).fail(function () {
    ...
    feature_log: function (feature) {
      $.log({ name: "feature", msg: feature });
    },
    structured_log: function (data) {
  ...
  (function (w) {
            ...
            if (ref) {
              ...
              if (m) {
                ...
                if (file && this.isSameDomain(file) && lineno) {
                  var functionName = this.guessAnonymousFunction(
                    file,
                    lineno,
                    charno
                  );
    ...
    var f = ["alert", "prompt", "confirm"];
    for (i = 0; i < f.length; i++) {
      w[f[i]] = (function (name, proxied) {
        return function () {
          var loggerUrl = "/api/xsslog/";
          var parameters =
            "?msg=" +
            escape(
  ...
  if (window.JSON === undefined) {
    var s = $("<script/>").attr({ src: _ASSET("/assets/js/lib/json2.js") });
    $("head").append(s);
  }
})(jQuery);
    ...
    jsonlog: function (params) {
      ...
      for (var i = 0; i < mandatoryFields.length; i++) {
        if (params[mandatoryFields[i]] === undefined) {
          isProduction ||
            console.warn(
              mandatoryFields[i] + " is missing from the parameter object"
            );
          return false;
        }
      }
      ...
      var defaults = {
        ...
        levelname: "INFO",
        levelno: 20,
        created: parseInt(params.current_time / 1e3, 10),
        msecs: params.current_time % 1e3,
        relativeCreated: 0,
        msg: "",
        source: source,
        prezi_id: prezi_id,
        ...
        putma_id: $.cookie && $.cookie("__putma"),
      };
      var data = {};
      ...
      var _log_request = function () {
        $.ajax({
          ...
          headers: { authorization: jsonLoggingConfig.header },
          dataType: "json",
          timeout: 2e3,
          ...
          error: function (xhr, textStatus) {
            ...
            if (
              typeof featureSwitch !== "undefined" &&
              featureSwitch &&
              featureSwitch.hasOwnProperty("testLoggingRetry")
            ) {
              retryAjax(this);
            }
          },
...
$(function () {
  $(".changelang_loggedout").click(function (event) {
    event.preventDefault();
    $.ajax({
      type: "POST",
      dataType: "json",
      ...
      data: { language: $(this).attr("data-langcode") || "en" },
      complete: function (request) {
        if (request.responseJSON && request.responseJSON.redirect_url) {
          window.location.replace(
            request.responseJSON.redirect_url + location.search
          );
        } else {
          location.reload();
        }
  ...
  function updateProfileLanguagePreference() {
    var stay_on_page = $(this).attr("stay-on-page") === "true";
    return $.ajax({
      type: "POST",
      dataType: "json",
      url: "/api/profile/",
      headers: { "X-CSRFToken": $.cookie("csrftoken") },
      data: { lang_preference: $(this).attr("data-langcode") },
      complete: function (request) {
        if (
          request.responseJSON &&
          request.responseJSON.redirect_url &&
          !stay_on_page
        ) {
          window.location.replace(
            request.responseJSON.redirect_url + location.search
  ...
  $(".changelang_loggedin").click(function (event) {
    event.preventDefault();
    if (
      typeof window.glassboxLogger !== "undefined" &&
      !!window.glassboxLogger.logChangeLanguage
    ) {
      var logAttributes = { target_language: $(this).attr("data-langcode") };
      window.glassboxLogger
        .logChangeLanguage(logAttributes)
        .finally(updateProfileLanguagePreference.call(this));
    } else {
      updateProfileLanguagePreference.call(this);
    }
  });
});

$(function () {
  $("a[data-logout]").click(function (event) {
    event.preventDefault();
    var target = $(event.target);
    var clickSource = target.data("click-source");
    var clickSourceParam =
      clickSource != null ? "?click_source=" + clickSource : "";
    var url = "/api/v2/auth/logout/" + clickSourceParam;
    $.ajax({
      type: "POST",
      url: url,
      dataType: "json",
      ...
      error: function () {
        $.log("Logout failed with the following url:" + url);
      },
    });
  });
  function cleanupBrowserData() {
    for (var key in window.localStorage) {
      if (key.startsWith("recenttopics.")) {
...
$(function () {
  if (typeof $.cookie == "function" && !$.cookie("upgrade_bt")) {
    $("#banner-top")
      .show()
      .on("click", ".close", function (e) {
        $("#banner-top").hide();
        $.cookie("upgrade_bt", 1, { expires: 7 });
      });
  }
});
function loadNotPreloadedVideos() {
  window.addEventListener("DOMContentLoaded", function () {
    var notPreloadedVideos = document.querySelectorAll(
      "video[preload='none'][data-load-later]"
    );
...
$(document).ready(function () {
  ...
  if ($ralewayHeader.length) {
    ...
    var $menuDropdown = $(
      ".show-menu-dropdown-on-hover .dropdown .dropdown-group"
    ).add(".show-menu-dropdown-on-hover .product-dropdown-container");
...
(function ($) {
  $.fn.ellocate = function (uniqueIds) {
    ...
    for (; el && el.nodeType == 1; el = el.parentNode) {
      ...
      if (el.tagName.substring(0, 1) != "/") {
        if (
          el.id != "undefined" &&
          el.id != "" &&
          eloc.isUniqueId(el.id, uniqIds)
        ) {
          uniqIds.push(el.id);
          var idPath = "[@id=" + "'" + el.id + "'" + "]";
          locator.xpath =
            "/" + el.tagName.toLowerCase() + idPath + locator.xpath;
          locator.css =
            el.tagName.toLowerCase() + "#" + el.id + " > " + locator.css;
        } else {
          idx = "[" + idx + "]";
          locator.xpath = "/" + el.tagName.toLowerCase() + idx + locator.xpath;
          locator.css =
            el.tagName.toLowerCase() + eloc.getClass(el) + " > " + locator.css;
        }
      }
    }
...
var GA_RANDOM_MIN = 1e9;
var GA_RANDOM_MAX = 9999999999;
var GA_SYNCHRONOUS_TIMEOUT_MILLISECONDS = 200;
var GA_SYNCHRONOUS_WAIT_MILLISECONDS = 100;
(function ($) {
  "use strict";
  var crumbleCookie = function (getKey) {
    var cookies = document.cookie.split(";");
    var kvCookies = {};
    for (var cookie = 0; cookie < cookies.length; cookie++) {
      var key = $.trim(
        cookies[cookie].substring(0, cookies[cookie].indexOf("="))
      );
      kvCookies[key] = $.trim(
        cookies[cookie].substring(
          cookies[cookie].indexOf("=") + 1,
          cookies[cookie].length
        )
      );
    }
    if (getKey) {
      return kvCookies[getKey] ? kvCookies[getKey] : null;
    } else {
      return kvCookies;
    }
  };
  var randomAnalyticsId = function () {
    return (
      "" +
      (Math.floor((GA_RANDOM_MAX - GA_RANDOM_MIN) * Math.random()) +
        GA_RANDOM_MIN)
    );
  };
  $.track = {};
  $.track.url = function (url) {
    if (!url) {
      return;
    }
    try {
  ...
  $.track.event = function (category, action, label, value, non_interaction) {
    if (value === undefined) {
      value = 0;
    }
  ...
  $.track.syncEvent = function (category, action, label, value) {
    var _gaProfileId = window._gaProfileId;
    var url = "http://www.google-analytics.com/__utm.gif";
    var cookies =
      "__utma=" +
      crumbleCookie("__utma") +
      ";__utmz=" +
      crumbleCookie("__utmz");
    action = action || "";
    label = label || "";
    if (value === undefined) {
      value = 0;
    ...
    var data = {
      utmwv: "4.8.9",
      utmn: randomAnalyticsId(),
      utmhid: randomAnalyticsId(),
      utmhn: window.location.hostname,
      utmr: "-",
    ...
    $.ajax({
      ...
      async: false,
      cache: false,
      timeout: GA_SYNCHRONOUS_TIMEOUT_MILLISECONDS,
    });
    return $;
  };
  $.fn.trackEvent = function (options) {
    ...
    var eventLabel = options["eventLabel"] || event;
    ...
    $(this).bind(event, function (e) {
      ...
      if (async) {
        $.track.event(category, eventLabel, label, value);
      } else {
        if ($this.is("a") && event === "click") {
          $.track.event(category, eventLabel, label, value);
          e.preventDefault();
          setTimeout(function () {
            document.location = $this.attr("href");
          }, GA_SYNCHRONOUS_WAIT_MILLISECONDS);
        } else {
          $.track.syncEvent(category, eventLabel, label, value);
        }
      }
    });
    return this;
...
jQuery(function ($) {
  "use strict";
  $("#navigation a").trackEvent({ category: "NavigationTabs", async: false });
  $("#userfunctions .sign-up").trackEvent({
    category: "SignUp",
    label: "global.header",
    async: false,
  });
  $("body.home  #content #signup").trackEvent({
    category: "SignUp",
    label: "coverPage.bigButton",
    async: false,
  });
  $("body.login #content .sign-up").trackEvent({
    category: "SignUp",
    label: "loginPage",
    async: false,
  });
  $('#content a[href="mailto:sales@prezi.com"]').trackEvent({
    category: "Link",
    label: "sales@prezi.com",
  });
});

var Prezi = Prezi || {};
var logElement = function (event) {
  var el = event.data.el;
  var source = $(el).attr("href");
  if (source && source.indexOf("?click_source=") > -1) {
    return;
  }
  if (source && source[0] === "/") {
    console.warn(
      'Please provide proper logging parameters for "' + source + '" link'
    );
  }
  var action = $(el).data("action") || event.data.action || "click";
  var object = $(el).data("object") || event.data.object || "";
  var targetUrl = $(el).data("target-url");
  var domPath = $(el).ellocate();
  var putmaId = $.cookie("__putma");
  var async = true;
  var dataAsync = String($(el).data("async"));
  if (dataAsync) {
    async = dataAsync === "false" ? false : true;
  }
  if (!targetUrl) {
    var targetUrlArray = document.location.href.split("/");
    targetUrlArray.splice(0, 3);
    targetUrl = "/" + targetUrlArray.join("/");
  }
  var jsonLogDict = {
    ...
    trigger: action,
    object: object,
    dom_path: domPath.css,
    source_url: document.location.pathname,
    target_url: targetUrl,
    putma_id: putmaId,
    platform: "website",
    user_id: Site.user_id ? parseInt(Site.user_id, 10) : 0,
  };
  if (object === "logged_element") {
    var pageLocation = $(el).data("page-location");
    if (pageLocation) {
      jsonLogDict["page_location"] = pageLocation;
    }
    var elementType = $(el).data("element-type");
    if (elementType) {
      jsonLogDict["element_type"] = elementType;
    }
    var elementText = $(el).data("element-text");
    if (elementText) {
      jsonLogDict["element_text"] = elementText;
    }
  }
  return $.jsonlog(jsonLogDict);
};
$(document).ready(function () {
  var taggedSelector = "[data-object]";
  $(taggedSelector).each(function () {
    $(this).bindFirst("click", { el: this }, logElement);
  });
});
